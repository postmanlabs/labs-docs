...
Suppose you are working on a request that is saved inside a [collection](/docs/postman/collections/creating_collections){:target="_blank"}. You can add examples to this request with a new custom response or the response from the server.
...
The ability to [save responses](/docs/postman/sending_api_requests/responses){:target="_blank"} has been available in Postman for a long time. However, our users wanted to edit these responses before saving them, and also add new responses. Examples provides this functionality.

You can save responses to examples. You save responses as before, but now you can edit them whenever you want. To access previously saved responses, click on the **Examples** dropdown. 
