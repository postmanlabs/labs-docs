...
Many Postman learning resources (including docs here in the Learning Center) use the Postman Echo API, because it provides a quick way to send a request without worrying about authentication or request configuration. If you want to learn how to do something in Postman without connecting to a "real" API, feel free to use the Postman Echo API at any time.
...
Make sure `GET` is selected in the method drop-down, and select __Send__.
...
The API returns a JSON response including the detail from the request you sent. If you use parameters or body data, it will also return those.
